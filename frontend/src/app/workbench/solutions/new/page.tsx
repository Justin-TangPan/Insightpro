...
function SolutionCreateForm() {
  ...
  return (
    <div className="page-stack">
      <SectionHeader badge={requirementId ? "背景材料 → 方案实践" : "Workbench / 方案实践"} title="创建方案实践" subtitle={requirementId ? "创建后会将当前背景材料自动注入 AI 上下文。" : "收纳可复用的方案背景，供 AI 工作区使用。"} />
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
        <form onSubmit={submit} className="ui-card space-y-5">
          ...
          <Field label="背景信息"><textarea rows={7} maxLength={5000} value={form.description} onChange={(event) => field("description", event.target.value)} className="ui-input w-full resize-y px-4 py-3" placeholder="业务背景、方案能力、适用场景和关键约束" /></Field>
