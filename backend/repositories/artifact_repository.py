...
def create(user_id: str, session: dict, artifact_id: str, title: str, artifact_type: str, content: str) -> dict:
    snapshot = session["context_snapshot"]
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            """INSERT INTO agent_artifacts (id, user_id, session_id, task_key, type, title, content, source_type, source_id, requirement_id, solution_id)
               VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *""",
            (artifact_id, user_id, session["id"], session.get("task_key", ""), artifact_type, title, content,
...
def list_for_user(user_id: str, limit: int = 30) -> list[dict]:
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id::text, session_id::text, task_key, type, title, source_type, source_id, knowledge_status, created_at, updated_at FROM agent_artifacts WHERE user_id=%s ORDER BY updated_at DESC LIMIT %s", (user_id, limit))
        return [dict(row) for row in cursor.fetchall()]


def get_for_user(user_id: str, artifact_id: str) -> dict | None:
    with get_db() as conn:
        cursor = conn.cursor(); cursor.execute("SELECT * FROM agent_artifacts WHERE id=%s AND user_id=%s", (artifact_id, user_id)); row = cursor.fetchone()
