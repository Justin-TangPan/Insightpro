...
主要模块：首页技术方案简报、技术热点、阿里云/华为云解决方案洞察、厂商动态、全局搜索、用户自有 Solutions 与 Requirements、后台 AI 摘要/评估/研报/采集加工、Scheduler 和 Health Check。
...
`aliyun_solutions` 是外部云厂商洞察数据（历史表名，当前含阿里云和华为云）；`solutions` 是按 `user_id` 隔离的用户自有方案；Requirements 可以从洞察创建并关联自有 Solution。
