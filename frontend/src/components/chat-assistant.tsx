    ...
    <>
      ...
      {open && (
        <div style={{
          ...
          {/* Header */}
          <div style={{ background: "var(--color-primary-dark)", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
            {messages.length === 0 && (
              <>
                <div style={{ display: "flex", gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, background: "var(--color-primary-dark)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  ...
                  <div style={{ background: "var(--color-surface-subtle)", borderRadius: "12px 12px 12px 4px", padding: "10px 14px", maxWidth: "85%" }}>
                    <p style={{ margin: 0, fontSize: 13, color: "var(--color-ink-secondary)", lineHeight: 1.6 }}>你好！我是 InsightPro 智能助手，可以回答技术热点、解决方案和友商能力等问题。</p>
                  </div>
                </div>
                <div style={{ paddingLeft: 38, display: "flex", flexDirection: "column", gap: 6 }}>
                  <p style={{ margin: 0, fontSize: 10, fontWeight: 600, color: "var(--color-ink-muted)", textTransform: "uppercase", letterSpacing: "0.05em" }}>快速提问</p>
                  {["首页洞察有哪些数据？", "热点追踪如何分析 GitHub 项目？", "阿里云解决方案最近有更新吗？", "如何对比友商方案能力？"].map((q, i) => (
                    <button key={i} onClick={() => { setInput(""); setMessages([{ role: "user", content: q }]); setLoading(true); fetch(`${API}/api/chat`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: q, history: [] }) }).then(r => r.json()).then(d => setMessages([{ role: "user", content: q }, { role: "assistant", content: d.reply || "暂无法回答" }])).catch(() => setMessages([{ role: "user", content: q }, { role: "assistant", content: "网络异常" }])).finally(() => setLoading(false)); }}
                      style={{ display: "block", width: "100%", textAlign: "left", fontSize: 12, color: "var(--color-primary)", background: "var(--color-primary-soft)", border: "none", padding: "8px 12px", borderRadius: 8, cursor: "pointer" }}>
                      {q}
                    </button>
                  ))}
                </div>
            ...
            {messages.map((m, i) => (
              <div key={i} style={{ display: "flex", gap: 10, flexDirection: m.role === "user" ? "row-reverse" : "row" }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: m.role === "user" ? "var(--color-primary)" : "var(--color-primary-dark)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                ...
                <div style={{ maxWidth: "85%", padding: "10px 14px", borderRadius: m.role === "user" ? "12px 12px 4px 12px" : "12px 12px 12px 4px", background: m.role === "user" ? "var(--color-primary)" : "var(--color-surface-subtle)", color: m.role === "user" ? "#fff" : "var(--color-ink-secondary)" }}>
            ...
            {loading && (
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <div style={{ background: "var(--color-surface-subtle)", borderRadius: "12px 12px 12px 4px", padding: "12px 16px", fontSize: 13, color: "var(--color-ink-muted)" }}>思考中...</div>
              </div>
            )}
            <div ref={endRef} />
          </div>
          ...
          {/* Input */}
          <div style={{ borderTop: "1px solid var(--color-grid)", padding: "12px 16px" }}>
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                placeholder="输入你的问题..."
                disabled={loading}
                style={{ flex: 1, padding: "8px 12px", borderRadius: 8, background: "var(--color-surface-subtle)", border: "1px solid var(--color-grid)", fontSize: 13, outline: "none" }}
              />
              <button onClick={send} disabled={loading || !input.trim()}
                style={{ width: 36, height: 36, borderRadius: 8, background: loading || !input.trim() ? "var(--color-grid)" : "var(--color-primary-dark)", color: "#fff", border: "none", cursor: loading || !input.trim() ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
