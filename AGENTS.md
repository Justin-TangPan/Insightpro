...
Insight-Agent may edit files only inside its isolated Git Workspace. It must not write business data, run shell commands, call side-effect APIs, deploy, push/merge Git, or access production directories. Never read or expose `.env`, credentials, tokens, production keys, or other secrets.
