...
def ensure_runtime_schema() -> None:
    with get_db() as conn:
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS runtime_settings (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())")
        cursor.execute("CREATE TABLE IF NOT EXISTS ai_usage_records (id BIGSERIAL PRIMARY KEY, user_id UUID NOT NULL, model TEXT NOT NULL, feature TEXT NOT NULL, input_tokens INTEGER NOT NULL DEFAULT 0, output_tokens INTEGER NOT NULL DEFAULT 0, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_ai_usage_records_created ON ai_usage_records(created_at DESC)")
