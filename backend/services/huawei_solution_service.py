...
def _parse_catalog(page: str) -> list[dict]:
    ...
    for category, url, title, description in CARD_RE.findall(page):
        ...
        seen.add(url)
        items.append({
            "title": _clean(html.unescape(title)), "url": url,
            "category": f"{_clean(html.unescape(category))} / 解决方案实践",
            "source_description": _clean(BeautifulSoup(html.unescape(description), "html.parser").get_text(" ")),
