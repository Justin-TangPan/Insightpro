...
Gateway 不能自行提供数据隔离，因此 Runtime Manager 为每个稳定 `user_id` 启动独立 Hermes Dashboard 子进程，并分配独立 Linux UID、端口、Workspace 和 `HERMES_HOME`。普通用户目录为所有者可写、管理组可管理，其他 UID 无访问权；公共知识库由管理员 UID 持有，普通用户只有读取权限。
